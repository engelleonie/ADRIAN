...
import tech.jorn.adrian.core.risks.PropertyBasedRule;
...
//            Files.createDirectories(path.getParent());
    ...
    public Set<String> applyProposal(AuctionProposal proposal) {
        Set<String> changedNodeIds = new HashSet<>();
        if (!(this.agentState.current().equals(AgentState.Idle) || (this.agentState.current().equals(AgentState.Ready)))) {
        ...
        if (mutation instanceof Migration<?, ?> migration) {
            changedNodeIds.add(migration.getAsset().getID());
            changedNodeIds.add(migration.getFrom().getID());
            changedNodeIds.add(migration.getNode().getID());
            ...
            infrastructureEffector.updateInfra(infra -> {
                if (mutation instanceof AttributeChange<?, ?> attributeChange) {
                    changedNodeIds.add(attributeChange.getNode().getID());
                    ...
                    changedNodeIds.add(softwareAttributeChange.getNode().getID());
