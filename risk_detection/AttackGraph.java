...
public class AttackGraph extends AbstractGraph<AttackGraphEntry<?>, AttackGraphLink<AttackGraphEntry<?>>> {
    ...
    public AttackGraph() {
        ...
        System.out.println("Nodes at start:");
        this.getNodes().forEach(n -> System.out.println(n.getID()));
        System.out.println("VoidNode incoming edges: " + this.getIncoming());
    }


    public void addEdge(AttackGraphEntry<?> from, AttackGraphEntry<?> to, Risk risk) {
        System.out.println("Adding edge: " + from.getID() + " -> " + to.getID() + " with risk " + risk.type());
        var adj = this.adjacent.getOrDefault(from, new ArrayList<>());
        System.out.println("Current adj list size before adding: " + adj.size());
        ...
        System.out.println("Adj list size after adding: " + this.adjacent.get(from).size());
    }

    @Override
    public void addEdge(AttackGraphEntry<?> from, AttackGraphEntry<?> to) {
        throw new UnsupportedOperationException(
                "Use addEdge(from, to, risk) instead — Risk must always be specified!");
    }


    public void addIncoming(AttackGraphEntry<?> to, Risk risk) {
        this.addEdge(VoidNode.getIncoming(), to, risk);
        System.out.println("VoidNode edges after addIncoming(" + to.getID() + "): "
                + this.adjacent.get(VoidNode.getIncoming()));
    }
